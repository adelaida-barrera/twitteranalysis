...
On the basis of tweets gathered from individual and institutional accounts, we offer here a mostly descriptive exploration of the public discourse around feminism and gender issues in the Colombian context. After some remarks about the selection of the sample, we identify the most relevant topics in our data with bi-grams and topic modelling. We then explore connections between the accounts and relative positions on some topics, with scaling. Finally, with sentiment analysis, we explore how these tweeters feel about certain controversial topics.  
...
This is a quite long report since, prior authorization by the instructor, it corresponds to the final paper of both the Intro to Data Science and Text as Data courses. The distribution of the work is as follows: 

**Intro DS:**

* R project and [GitHub](https://github.com/adelaida-barrera/twitteranalysis) setup
* Twitter scraping 
* Data cleaning (including regular expressions)
* Scaling 
...
<br>
<br> 
After having run a some topic models with a random sample of 7000 tweets from this subset, we realized congress women (which we had selected for their prominence in the feminist agenda) were creating a lot of noise since they mainly use their accounts for political campaigns and to show cast their work in all sorts of issues. So we decided to remove them from the sample.
...
To identify some broad categories or topics that these women tweet about, we used the `stm()` function from the `stm` package to estimate a structural topic model. Behind this technique lies the assumption that the words generated in the tweets (which we can observe) were chosen by the tweeters when intending to talk about a topic (which is latent, unobserved). Stm is an unsupervised model that infers topics from the text  –the tweets are not coded into topics or categories by hand. “Topics” therefore are do not defined as a clear cut subset of words, but as a distribution over the ‘bag of words’ that compose the corpus of tweets: for each topic, each word has a probability of being part of it –we can gather a matrix of all words present in the corpus with the corresponding proportions of them that compose each topic (“betas”) ; this allows to us to see the words that “contribute” most to each topic. Additionally in this model each document is represented as a combination of topics and for each document we can see a matrix of what proportion of each document corresponds to each topic (“gammas”). (See [Roberts et al., 2014](https://authors.library.caltech.edu/52278/7/topicmodelsopenendedexperiments.pdf)). This technique essentially infers a lot of information solely based on the count of words observed from each document and the resercher’s specification of **how many topics** it should look for (also see Roberts  et al about how there is no “right” answer for this). 
...
To identify some broad categories or topics that these women tweet about, we used the `stm()` function from the `stm` package to estimate a **structural topic model**. Behind this technique lies the assumption that the words generated in the tweets (which we can observe) were chosen by the tweeters when intending to talk about a topic (which is latent, unobserved). STM is an unsupervised model that infers topics from the text  –so the tweets are not coded into topics or categories by hand. “Topics” therefore are not defined as a clear cut subset of words, but as a distribution over the ‘bag of words’ that compose the corpus of tweets: each word is assigned a probability of being part of topic. So, from the model we can gather a matrix (“beta”) of all words present in the corpus with the corresponding proportions of them that compose each topic; this allows to us to see the words that “contribute” most to each topic. Additionally in this model each document is represented as a combination of topics: for each document we can see a matrix (“gamma”) of what proportion of it corresponds to each topic. (See [Roberts et al., 2014](https://authors.library.caltech.edu/52278/7/topicmodelsopenendedexperiments.pdf)). This technique essentially infers a lot of information solely based on the count of words observed from each document and the resercher’s specification of **how many topics** it should look for (also see Roberts  et al about how there is no “right” answer for this). 

In practical terms, we ran this STM by turning our ‘dfm’ (matrix of counts of words in each document) into an stm corpus and [ran our topic models](https://github.com/adelaida-barrera/twitteranalysis/blob/main/topic_model_fem_full.Rmd) independently for individual and institutional accounts. We tried out different specifications of how many topics we should have on a random sample taken from the large dataset we had; we prioritized the interpretability of such topics and defined 10 as the right number of topics. Based on the most likely words to be in each category, we gave a human semantic interpretation to them and adjudicated labels to them. Yet, the model did not perfectly classify the documents according to our semantic interpretation: when looking into documents classified as predominantly from a certain topic, we found tweets that a human classifier would definitely put in a different basket. But, all in all, what we got seemed reasonable.  

### Topics in institutional accounts

To take a closer look at the topics we identified, we gathered the most prevalent topic in each tweet (the highest “gamma”) and could see how many documents were mainly talking about each topic.
...
Lacadavidc, an left-wing 'raical feminist' activist, distances from the group positioning beyond -3, this could be due for her polemic positions against the women trans movement. In contrast, we observe alejaoficial, who positions almost in 1, she is a famous artist who advocates against women violence. Also at the right side of the scale is Angela Anzola, a right-wing former head of the Womens' Secretariat in Bogotá. The scale shows these  women talk about political issues it using different vocabulary. 
...
The institutional accounts scale discussing reproductive rights topics show us an interesting account grouping. The accounts close to 0 and the positive quadrant are institutions that advocate for women´s rights in a very 'isntitutionalized' and general manner: Women_ Equity, Women Commission Colombia, Women Secretary and ONU Women. In the middle, between -2 and 0, the spectrum opens more and sems organized by bundles of issues. From right to left,  we can find activits from political movements, LGBT activists and pro-choice organizations dealing with abortion debates. The last groups  to the lefy are the Constitutional Court and the Truth Commission, we believe they are the extreme left because they use a different language compared to the other institutions or activists accounts, and they discuss a variety of topics regarding reproductive rights beyond women.  
