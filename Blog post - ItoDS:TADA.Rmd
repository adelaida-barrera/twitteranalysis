...
Exploring the results with the first dictionary we didn´t see a significant difference between the positive and negative score. So, we decided to perform a qualitative analysis of the dictionary, we found that the most frequent words used by the accounts in each topic (information gathered in the topic model section) were not included in the dictionary we selected. Thus, the words were included in the dictionary to improve the analysis. The results after running again the sentiment analysis improved.  
...
+The quality of the dictionary is essential for the sentiment analysis. 
+There are more developments in this area in languages like English than Spanish, so it is important to verify the content of the dictionary or compare the results of more than one dictionary to select the best one.
+Using the improved dictionary, we can conclude that overall the personal and institutional accounts use a positive language. 
